...
print("🚀 Launching Version 2.2: Fully Hardened Institutional Risk Surveillance Engine...")

# ============================================
# 1. HARDENED ADDRESS NORMALIZATION ENGINE
# ============================================
def normalize_address_hardened(item, boro_name):
    # Cross-reference alternate column variants used across different NYC open datasets
    ...
    # If the record completely lacks specific location telemetry, mark as a generic asset
    if num == "" or street == "":
        return "Unknown Address, NYC Multifamily Asset"
...
try:
    ...
    if isinstance(lit_data, list) and len(lit_data) > 0:
        for item in lit_data:
            ...
            addr = normalize_address_hardened(item, boro_name)
            ...
            if "HARASSMENT" in str(case_type).upper():
                properties_tracked[addr]["raw_points"] += 30  
            else:
                properties_tracked[addr]["raw_points"] += 20  
    ...
    if isinstance(viol_data, list) and len(viol_data) > 0:
        for item in viol_data:
            ...
            if "HAZARDOUS" in severity or "CLASS 1" in severity:
                properties_tracked[addr]["raw_points"] += 35  
            else:
                properties_tracked[addr]["raw_points"] += 15  
except Exception as e:
    print(f"⚠️ Violations bypass: {e}")

# ============================================
# 3. MATHEMATICAL SCORING & NORMALIZATION ENGINE
# ============================================
endangered_assets = []
for addr, details in properties_tracked.items():
    # Hard cap total points at 100 to ensure mathematical scale soundness
...
try:
    response = client.chat.completions.create(
        ...
        max_tokens=800,
        temperature=0.2  # Low temperature keeps responses factually tight and uniform
    )
    print("\n========================================================")
    print("📋 RE SURVEILLANCE REPORT: VERSION 2.2 (FULLY CONSOLIDATED)")
    print("========================================================")
